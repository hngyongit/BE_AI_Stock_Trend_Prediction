...
| **Analyse Service Scaffold** |  Skeleton | Python + FastAPI + OpenAI SDK | Thư mục `analyse` đã chuẩn bị schema, prompt, route placeholder và client stub; chưa gọi OpenAI thật. |
...
Luồng mở rộng cho tầng phân tích AI/LLM mới:

```text
crawler → MongoDB → api → analyse → frontend/client
```

Trong luồng này, `analyse` là service Python riêng ở port `5100` mặc định. Service này hiện chỉ là scaffold để chuẩn bị cho bước sau: nhận dữ liệu từ client hoặc fetch từ `api`, chuẩn hóa payload, tạo prompt, gọi OpenAI và trả báo cáo phân tích tiếng Việt dạng JSON.

---

## 4. Repository Structure

Cấu trúc thư mục chính ở root hiện tại:

```text
BE_AI_Stock_Trend_Prediction/
├── api/                              # Dịch vụ Backend API (Node.js/Express)
│   ├── src/
│   │   ├── app.js                    # Khởi tạo Express, cấu hình middleware và router chính
│   │   ├── server.js                 # Khởi động server, kết nối MongoDB và chạy seed role/user
│   │   ├── config/                   # Cấu hình CORS, DB, JWT, Passport OAuth, Swagger
│   │   ├── common/                   # Middleware, store và response helpers
│   │   ├── database/
│   │   │   ├── models/               # Mongoose schemas cho Dim/Fact/Operational collections
│   │   │   └── seeds/                # Seed roles, users, markets, industries
...
│   └── requirements.txt
│
└── analyse/                          # Python AI/LLM analysis scaffold, chạy riêng port 5100
    ├── src/analyse/
    │   ├── api/                      # FastAPI routes/controllers placeholder
    │   ├── clients/                  # Backend API client và OpenAI client skeleton
    │   ├── prompts/                  # Prompt templates cho phân tích tiếng Việt
    │   ├── schemas/                  # Pydantic request/response schemas
    │   └── services/                 # Stub normalize/analyse/risk/portfolio plan
    ├── tests/
    ├── .env.example
    ├── requirements.txt
    └── run.py
...
`analyse` là scaffold Python cho tầng phân tích AI/LLM trong tương lai. Ba phần chính của hệ thống hiện là:

* `api`: REST API Node.js/Express/Mongoose, đọc MongoDB và cung cấp stock, watchlist, dashboard, auth và subscription.
* `crawler`: crawler Python thu thập dữ liệu Vietstock, ghi trực tiếp vào MongoDB.
* `analyse`: service Python/FastAPI chuẩn bị cho phân tích bằng OpenAI, hiện chỉ có route placeholder và schema/stub.

Các endpoint placeholder của `analyse`:
...
Cài đặt và chạy skeleton trên Windows PowerShell:
...
Cài đặt và chạy skeleton trên Bash/macOS/Linux:
...
Gửi request phân tích một mã cổ phiếu ở chế độ placeholder:
...
`analyse/.env.example` đã chuẩn bị các biến:
...
Không đưa API key thật vào Git, log, ảnh chụp màn hình hoặc response trả về client. `analyse` hiện chưa gọi OpenAI thật; đây chỉ là skeleton. Chất lượng phân tích sau này sẽ phụ thuộc vào dữ liệu backend trả về, đặc biệt là `latest_price`, chart OHLCV, `market_overview`, financials và thông tin chất lượng crawl.
